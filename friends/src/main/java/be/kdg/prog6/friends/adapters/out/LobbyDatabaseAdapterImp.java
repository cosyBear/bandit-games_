...
public class LobbyDatabaseAdapterImp implements LobbySavePort, LobbyLoadPort {
    ...
    public Lobby loadLobbyById(LobbyId lobbyId) {
        Optional<LobbyJpaEntity> entity = lobbyJpaRepository.findById(lobbyId.id());
        if (entity.isPresent()) {
            return entity.get().toDomain();
        } else
            throw new EntityNotFoundException("Lobby was not found for id " + lobbyId.id());
    }
    ...
    public void save(Lobby domain) {
        ...
        final LobbyJpaEntity lobbyJpaEntity = LobbyJpaEntity.fromDomain(domain, playerJpaEntity, domain.getLobbyStatus());
