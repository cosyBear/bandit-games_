...
public class LibraryDatabaseAdapter implements LibraryLoadPort, LibrarySavePort {
    ...
    public Library fetchLibraryWithAllAvailableGames(PlayerId playerId) {
        return Mapper.mapDomainLibrary(libraryJpaRepository.fetchLibraryWithAllAvailableGames(playerId.Id()));
    }

    @Override
    public Library fetchLibraryWithGamesByNamePattern(PlayerId playerId, String gameName) {

        return Mapper.mapDomainLibrary(libraryJpaRepository.fetchLibraryWithGamesByNamePattern(playerId.Id(), gameName));
    }

    @Override
    public Library fetchLibraryWithGamesByCategory(PlayerId playerId, GameType category) {
        return Mapper.mapDomainLibrary(libraryJpaRepository.fetchLibraryWithGamesByCategory(playerId.Id(), GameTypeEntity.valueOf(category.toString())));
    }
